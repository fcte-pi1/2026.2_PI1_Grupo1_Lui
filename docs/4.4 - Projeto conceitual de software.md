...
## Arquitetura do Firmware de Navegação Autônoma

Abaixo está o diagrama de atividades UML (fluxograma de decisão) representando a lógica de navegação autônoma do robô:

```mermaid
flowchart TD
    A([Início da Navegação]) --> B{Obstáculo Detectado?}
    B -- Sim --> C[Calcular Rota de Desvio]
    C --> D[Ajustar Trajetória]
    D --> E[Continuar Movimento]
    B -- Não --> E
    E --> F{Chegou ao Destino?}
    F -- Sim --> G([Finalizar Navegação])
    F -- Não --> B
```

## Mockup e Estrutura do Painel Web de Telemetria

### Esboço (Wireframe/Mockup) do Dashboard Web

O painel web permite monitorar o robô em tempo real, visualizando suas métricas vitais e seu deslocamento.

```text
+---------------------------------------------------------------+
|  [Logo]   Painel Web de Telemetria                 [Perfil]   |
+---------------------------------------------------------------+
|                                                               |
|  +-----------------------+  +------------------------------+  |
|  | Status do Robô        |  | Mapa de Navegação / Câmera   |  |
|  |                       |  |                              |  |
|  | - Bateria: 85%        |  |                              |  |
|  | - Velocidade: 1.2 m/s |  |         [ MAPA ]             |  |
|  | - Estado: Em Movimento|  |                              |  |
|  | - Conexão: Estável    |  |                              |  |
|  +-----------------------+  +------------------------------+  |
|                                                               |
...
|  | [14:30:00] Iniciou navegação para Coordenada X, Y       |  |
|  | [14:32:15] Obstáculo detectado. Recalculando rota...    |  |
|  | [14:32:18] Rota ajustada com sucesso.                   |  |
|  +---------------------------------------------------------+  |
+---------------------------------------------------------------+
```
...
A arquitetura de dados (banco de dados relacional) projetada para suportar a telemetria e o gerenciamento de usuários é ilustrada no Diagrama Entidade-Relacionamento abaixo:

```mermaid
erDiagram
    ROBOT ||--o{ TELEMETRY : envia
    USER ||--o{ ROBOT : monitora
